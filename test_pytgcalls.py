import os
from pathlib import Path
from typing import Optional

import numpy as np
from dotenv import load_dotenv
from pytgcalls import PyTgCalls, filters, idle
from pytgcalls import filters as fl
from pytgcalls.types import (
    ChatUpdate,
    Device,
    Direction,
    ExternalMedia,
    GroupCallConfig,
    GroupCallParticipant,
    MediaStream,
    RecordStream,
    StreamEnded,
    StreamFrames,
    Update,
    UpdatedGroupCallParticipant,
)
from pytgcalls.types.raw import AudioParameters
from telethon import TelegramClient, events

load_dotenv()

app = TelegramClient(
    "py-tgcalls",
    api_id=int(os.environ["TELEGRAM_API_ID"]),
    api_hash=os.environ["TELEGRAM_API_HASH"],
)
call_py = PyTgCalls(app)

SAMPLE_MEDIA = str(Path.home() / "Downloads" / "sample-15s.wav")
SAMPLE_IMAGE = str(Path.home() / "Downloads" / "newplot.png")

AUDIO_PARAMETERS = AudioParameters(bitrate=48000, channels=2)


async def _start_recording(
    chat_id: int,
    *,
    enable_video: bool,
    invite_hash: Optional[str] = None,
):
    stream = RecordStream(
        audio=True,
        audio_parameters=AUDIO_PARAMETERS,
        camera=enable_video,
        screen=enable_video,
    )

    config = (
        GroupCallConfig(invite_hash=invite_hash, auto_start=False)
        if enable_video
        else None
    )

    await call_py.record(chat_id, stream, config=config)


async def play_sample(chat_id: int, media_url: Optional[str] = None):
    await call_py.play(
        chat_id,
        MediaStream(media_url or SAMPLE_MEDIA, video_flags=MediaStream.Flags.IGNORE),
    )


async def show_image(chat_id: int, image_url: str):
    is_group = chat_id < 0
    config = GroupCallConfig(auto_start=True) if is_group else None
    await call_py.play(
        chat_id,
        MediaStream(
            image_url,
            audio_flags=MediaStream.Flags.IGNORE,
        ),
        config=config,
    )


async def send_chat_message(chat_id: int, message: str):
    await app.send_message(chat_id, message)


@app.on(events.NewMessage(pattern="!image"))
async def image_handler(event):
    chat_id = event.chat_id
    print(f"[+] !image requested in {chat_id}, publishing static image...")
    await show_image(chat_id, SAMPLE_IMAGE)


@call_py.on_update(filters.chat_update(ChatUpdate.Status.INCOMING_CALL))
async def incoming_call_handler(_: PyTgCalls, update: Update):
    chat_id = update.chat_id
    print(f"[+] Incoming call from {chat_id}, answering...")

    await call_py.play(chat_id, MediaStream(ExternalMedia.AUDIO, AUDIO_PARAMETERS))
    await _start_recording(chat_id, enable_video=False)

    print(f"[+] Call answered and recording started for chat {chat_id}")


@app.on(events.NewMessage(pattern="!play"))
async def play_handler(event):
    chat_id = event.chat_id
    print(f"[+] !play requested in {chat_id}, streaming sample media...")

    is_group = chat_id < 0
    config = GroupCallConfig(auto_start=True) if is_group else None

    await call_py.play(chat_id, SAMPLE_MEDIA, config=config)


@call_py.on_update(filters.stream_frame(Direction.INCOMING, Device.MICROPHONE))
async def on_audio_chunk(_: PyTgCalls, update: StreamFrames):
    if not update.frames:
        return

    frame_bytes = update.frames[0].frame
    samples = np.frombuffer(frame_bytes, dtype=np.int16)
    print(samples[:5])


@call_py.on_update(
    filters.stream_frame(Direction.INCOMING, Device.CAMERA | Device.SCREEN)
)
async def on_video_frame(_: PyTgCalls, update: StreamFrames):
    if not update.frames:
        return

    frame = update.frames[0]
    print(f"[video] Chat {update.chat_id} frame size={len(frame.frame)} bytes")


@call_py.on_update(fl.stream_end())
async def stream_end_handler(_: PyTgCalls, update: StreamEnded):
    print(f"Stream ended in {update.chat_id}", update)


@call_py.on_update(fl.call_participant(GroupCallParticipant.Action.JOINED))
async def participant_handler(_: PyTgCalls, update: UpdatedGroupCallParticipant):
    chat_id = update.chat_id

    print(
        f"[+] Detected group call activity in {chat_id} (participant joined); "
        "joining and recording...",
    )

    await _start_recording(chat_id, enable_video=True)


if __name__ == "__main__":
    call_py.start()  # type: ignore
    print("[*] Running. Call this account from another Telegram client to test.")
    idle()
